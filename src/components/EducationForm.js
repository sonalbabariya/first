...
function EducationForm({ educationDetails, onChange, onSubmit }) {
    const [error , setError] = useState({});

    const validatForm = () => {
        const Error = {};
        if(!educationDetails.degree){
            Error.degree = "⚠️ Please Enter Your Degree...";
        }
        if(!educationDetails.university){
            Error.university = "⚠️ Please Enter Your University/Institution Name...";
        }
